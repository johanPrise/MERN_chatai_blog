...
const Chatbot = () => {
  ...
  const handleSend = async () => {
    if (input.trim() !== "") {
      ...
      try {
        ...
        if (response.ok) {
          setMessages((prevMessages) => [...prevMessages, { text: formatMessage(data.response), sender: "model" }])
        } else {
  ...
  return (
    <div className="fixed bottom-5 right-5 flex flex-col items-end">
      ...
      <div
        className={`transition-opacity duration-300 ease-in-out ${isOpen ? "opacity-100" : "opacity-0 pointer-events-none"}`}
      >
        {isOpen && (
          <div className="mt-3 w-80 h-96 flex flex-col bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden transform transition-transform duration-300 ease-in-out">
            <div className="flex justify-between items-center bg-lime-500 p-3 text-white">
              <span>iwopal</span>
              <button onClick={toggleChat} className="hover:bg-lime-600 p-1 rounded-full transition duration-300">
                <AiOutlineClose />
              </button>
            </div>
